  ...
  getStorage() {
    ...
    this.http.get<IElement[]>('/api/element', { params: params }).subscribe({
      next: (result) => {
        ...
        this.selectedElement = this.elementList.find(
          (element) => element.element_id === this.elementValue.element_id,
        )!;
        this.selectedElement.selected = true;
        this.elementList = this.elementList.filter(
          (element) => element.element_id !== this.selectedElement.element_id,
        );
      },
      error: (error) => {
        console.error(error);
